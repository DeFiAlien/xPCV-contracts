...
contract Controller {
    ...
    function balanceOf(address _vault)
        external
        view
        returns (uint256)
    {
    ...
    function farm(address _vault, uint256 _amount) external {
    ...
    function setStrategy(address _vault, address _strategy) external {
        require(msg.sender == governance, "!governance");
        require(
            IStrategy(_strategy).want() == IVault(_vault).token(),
            "unmatching want tokens between vault and strategy"
        );
